...
namespace RainbowMage.OverlayPlugin.MemoryProcessors
{
  public class FFXIVProcessCn : FFXIVProcess {
    ...
    internal override void ReadSignatures() {
      ...
      if (p.Count == 0) {
        logger_.Log(LogLevel.Error, "Charmap signature found {0} matches", p.Count);
      } else {
      ...
      if (p.Count != 1) {
        logger_.Log(LogLevel.Error, "Job signature found {0} matches", p.Count);
      } else {
      ...
      if (p.Count != 1) {
        logger_.Log(LogLevel.Error, "In combat signature found {0} matches", p.Count);
      } else {
        in_combat_addr_ = p[0];
      }
    }
